...
const port = process.env.PORTWEB || 8087;
...
// o [] deve estar dando erro, ver isso depois
...
app.post("/registerUser" , async ( req, res ) => {
    const reqb = req.body;
    // bug isAuthor and password
    let createU = await createUser(reqb.fullname, reqb.email, reqb.isAuthor, reqb.password);
    res.redirect("/login");
});
...
// Functions
async function createUser(name: any, email: any, isAuthor: any, password: any) {
    await prisma.$connect();
...
async function sendJson(){
    ...
    await prisma.dataset.create({
        data: {
            ...
            clusters : [
              { "key": "0", "color": "#6c3e81", "clusterLabel": "Information Systems" },
              { "key": "1", "color": "#666666", "clusterLabel": "Agronomy" },
              { "key": "3", "color": "#57a835", "clusterLabel": "Veterinary" },
              { "key": "4", "color": "#7145cd", "clusterLabel": "Chemistry" },
              { "key": "5", "color": "#579f5f", "clusterLabel": "Agricultural Sciences" }
            ],
            tags : [
